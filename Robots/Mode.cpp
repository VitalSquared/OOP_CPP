...
list<pair<int, int>> Mode::buildPath(int rs, int cs, int rf, int cf) {
    ...
    list<pair<int,int>> init;
    init.emplace_back(make_pair(rs, cs));
    q.push(init);

    while (!q.empty()) {
        list<pair<int,int>> cur = list<pair<int,int>>(q.front());
        q.pop();

        if (visited.find(cur.back()) != visited.end())
            continue;

        int r = cur.back().first, c = cur.back().second;

        if (make_pair(rf, cf) == cur.back())
            return cur;

        visited.insert(make_pair(r, c));

        pair<int,int> sccs[4] = { {r, c - 1}, {r, c + 1}, {r - 1, c}, {r + 1, c} };
        for (auto scc : sccs) {
            if (visited.find(scc) == visited.end() && validateCell(scc.first, scc.second)) {
                list<pair<int, int>> new_list(cur);
                new_list.emplace_back(make_pair(scc.first, scc.second));
                q.push(new_list);
            }
        }
    }
